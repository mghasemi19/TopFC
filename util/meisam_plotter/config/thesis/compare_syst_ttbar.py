...
if True:
    labels=["t#bar{t} nominal","t#bar{t} Rad-High","t#bar{t} Rad-Low"]
    to_compare=[["ttbar_NoSys"],["ttbar_rad_high_NoSys"],["ttbar_rad_low_NoSys"]]
    folder_out="plots/ttbar_syst_rad/"

if True:
    labels=["POWHEG","AMC@NLO"]
    to_compare=[["ttbar_PhHppEG_NoSys"],["ttbar_aMcAtNlo_NoSys"]]
    folder_out="plots/ttbar_syst_gen/"

if True:
    labels=["t#bar{t} nominal","HERWIG++"]
    to_compare=[["ttbar_NoSys"],["ttbar_PhHppEG_NoSys"]]
    folder_out="plots/ttbar_syst_ps/"
...
for var in var_def:
    ...
    for sel in selections:
        if "1L" in sel:
            ...
            write = write_0L
        compare (var['def'], selections[sel], wei_list, to_compare, name_infile_list, labels, var['leg'], lumi, logY, write, folder_out+sel+"/", name_can, do_scale=True, is_first_data=False)
