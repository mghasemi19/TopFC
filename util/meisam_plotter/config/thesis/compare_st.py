...
myparser.add_argument('++scale', default=0, type=int)
...
var_def += [{'def':("dphi_min",30,0,3),'leg':"#Delta#phi^{min}_{4j}(j,MET)"}]
"""
...
if syst==0:
    labels=["DR","DS"]
    to_compare=[["SingleTop_NoSys"],["SingleTop_DS_NoSys"]]
    folder_out="plots/st_DRDS/"
    weights_1 = weights_nom 
...
name_infile_DS = "/nfs/pic.es/user/c/crizzi/scratch2/susy_multib/HF_inputs/SUSYHF_tag2.4.28/Bkg_2.4.28-0-0_singletop_DS.root"
...
for i in range(len(labels)):
    ...
    for tc in to_compare[i]:
        if i==0: # DR
            ...
            name_infile_1[tc] = name_infile_DS
