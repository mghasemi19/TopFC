...
write_0L=["C. Rizzi PhD Thesis","Simulation, #sqrt{s}=13 TeV, 36.1 fb^{-1}","0 lepton, #geq 3 b-jets, #geq 4 jets,", "E_{T}^{miss} > 200 GeV, #Delta#phi^{4j}_{min}>0.4"]
...
folder_out="plots/sig_bkg_ewk/"
if True:
    labels=["Background","m(#tilde{H})=200 GeV","m(#tilde{H})=400 GeV","m(#tilde{H})=800 GeV"]
    to_compare=[["ttbar_NoSys","Wjets_NoSys","Zjets_NoSys","diboson_NoSys","SingleTop_NoSys","TopEW_NoSys","multijet_NoSys"],["GGM_hh_200_NoSys"],["GGM_hh_400_NoSys"],["GGM_hh_800_NoSys"]]
...
for i in range(len(labels)):
    ...
    for tc in to_compare[i]:
        if "multijet" in tc:
            name_infile_1[tc] = name_infile_qcd
        else:
            if i ==0:
...
for var in var_def:
    ...
    for sel in selections:
        ...
        if "1L" in sel:
            ...
            write = write_01L
        compare (var['def'], selections[sel], wei_list, to_compare, name_infile_list, labels, var['leg'], lumi, logY, write, folder_out+sel+"/", name_can, do_scale=False, is_first_data=False, x1leg = 0.60 , y1leg = 0.61, do_ratio=False)
