...
#sel_1L = "met>200 && jets_n>=5 && signal_leptons_n>=1 && pass_MET"
sel_0L = "(dphi_min>0.4) && (jets_n>=4) &&  (met>200) && (signal_leptons_n==0) && pass_MET"
name_infile = "/afs/cern.ch/user/c/crizzi/storage/susy_EW/eos/atlas/user/c/crizzi/susy_EW/HF_inputs/tag.EW.2.4.28-4-1/Sig_GGM_tagEW.2.4.28-4-1_SUSY10_nominal.root"
...
signals = ["GGM_hh_150","GGM_hh_300","GGM_hh_500","GGM_hh_800"]
...
for s in signals:
    for var in var_def:
        ...
        sel = sel_0L
        write=["#bf{#it{ATLAS}} Internal",s+" , 0L, >=4J, MET>200 GeV, dphi>0.4","36 fb^{-1}"]
        tt_dt (var['def'], sel, s, name_infile, var['leg'], 2, isIncl, lumi, logY, write+["2b Inclusive"], folder_out, name_can+"_tt_in2b_0L", do_scale, bvar)
        tt_dt (var['def'], sel, s, name_infile, var['leg'], 3, isIncl, lumi, logY, write+["3b Inclusive"], folder_out, name_can+"_tt_in3b_0L", do_scale, bvar)
        tt_dt (var['def'], sel, s, name_infile, var['leg'], 4, isIncl, lumi, logY, write+["4b Inclusive"], folder_out, name_can+"_tt_in4b_0L", do_scale, bvar)
