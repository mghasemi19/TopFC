...
var_def += [{'def':("newnonTopMass",20, 50, 300),'leg':"new-algorithm non-SM M_{top}"}]
var_def += [{'def':("testnonTopMass",20, 50, 300),'leg':"test new-algorithm non-SM M_{top}"}]
...
for var in var_def:
    ...
    topmass_plotter(var['def'], sel, myweights, backgrounds, signals, name_infile, labels, var['leg'],
                lumi, logY, write, folder_out, name_can)
