...
void tW_significance_compute()
{
   ...
   double tWcharm_sExpected_list[] = {15.6, 13.0, 10.4};       
   double tWcharm_bExpected_list[] = {40.7, 22.2, 11.4};   

   double relativeBkgUncert[] = {0.25, 0.5, 1};
   double tWcharm_pval_25[3][9];
   double tWcharm_zval_25[3][9];
   double tWcharm_pval_50[3][9];
   double tWcharm_zval_50[3][9];
   double tWcharm_pval_100[3][9];
   double tWcharm_zval_100[3][9];
   ...
   for (int i=0; i<3; i++){         // loop over sig/bkg events
      for (int k=0; k<3; k++){      // loop over bkguncert
         for (int j=0; j<9; j++){   // loop over mu_sig
	         double pExp = NumberCountingUtils::BinomialExpP(tWcharm_sExpected_list[i]*mu_sig[j], tWcharm_bExpected_list[i], relativeBkgUncert[k]);
	         double zExp = NumberCountingUtils::BinomialExpZ(tWcharm_sExpected_list[i]*mu_sig[j], tWcharm_bExpected_list[i], relativeBkgUncert[k]);
            //cout << "expected p-value =" << pExp << "  Z value (Gaussian sigma) = " << zExp << endl;
            if (relativeBkgUncert[k]==0.25){              
               tWcharm_pval_25[i][j] = pExp;
               tWcharm_zval_25[i][j] = zExp;
            }
            if (relativeBkgUncert[k]==0.5){
               tWcharm_pval_50[i][j] = pExp;
               tWcharm_zval_50[i][j] = zExp;
            }
            if (relativeBkgUncert[k]==1){
               tWcharm_pval_100[i][j] = pExp;
               tWcharm_zval_100[i][j] = zExp;
            }                        
   ...
   cout << "P-val for tWcharm_pval_25:" << endl;
   for (auto &row : tWcharm_pval_25){
      for (auto &col : row) {cout << col << endl;}
   }     
   cout << "Z-val for tWcharm_zval_25:" << endl;
   for (auto &row : tWcharm_zval_25){
      for (auto &col : row) {cout << col << endl;}
   }     

   cout << "P-val for tWcharm_pval_50:" << endl;
   for (auto &row : tWcharm_pval_50){
      for (auto &col : row) {cout << col << endl;}
   }     
   cout << "Z-val for tWcharm_zval_50:" << endl;
   for (auto &row : tWcharm_zval_50){
      for (auto &col : row) {cout << col << endl;}
   }

   cout << "P-val for tWcharm_pval_100:" << endl;
   for (auto &row : tWcharm_pval_100){
      for (auto &col : row) {cout << col << endl;}
   }     
   cout << "Z-val for tWcharm_zval_100:" << endl;
   for (auto &row : tWcharm_zval_100){
      for (auto &col : row) {cout << col << endl;}
   }   
